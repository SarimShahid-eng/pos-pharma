     ...
     <div @click.away="closeModal()"
         ...
         <div class="px-5 py-4 bg-paper border-b border-line flex items-center justify-between">
             <div>
                 ...
                 <h3 class="font-display text-lg font-bold text-forest" x-text="activePurchase?.invoice_number"></h3>
             </div>
             ...
             <div class="grid grid-cols-2 gap-4 p-4 bg-paper border border-line rounded-[var(--radius-s)] text-xs">
                 <div>
                     ...
                     <span class="font-bold text-ink text-sm block"
                         x-text="activePurchase?.supplier?.name || 'N/A'"></span>
                     <span class="font-mono text-muted block"
                         x-text="activePurchase?.supplier?.phone_number || 'No Phone'"></span>
                 </div>
                 <div class="text-right">
                     ...
                     <span class="font-mono text-ink block font-medium"
                         x-text="formatDate(activePurchase?.created_at)"></span>
                 </div>
             </div>
                 ...
                 <table class="w-full text-left text-xs text-ink">
                     <thead class="bg-paper border-b border-line font-semibold text-muted uppercase tracking-wider">
                         <tr>
                             ...
                             <th class="py-2.5 px-3 text-center">BonusQty</th>
                             <th class="py-2.5 px-3 text-right">Unit Cost</th>
                             <th class="py-2.5 px-3 text-right">Subtotal</th>
                         </tr>
                     </thead>
                     <tbody class="divide-y divide-line">
                         <template x-for="item in activePurchase?.purchase_items" :key="item.id">
                             <tr class="hover:bg-paper/40 transition-colors">
                                 ...
                                 <td class="py-2.5 px-3 text-right font-mono" x-text=" formatMoney(item.bonus_qty)">
                                 </td>
                                 <td class="py-2.5 px-3 text-right font-mono"
                                     x-text="'Rs ' + formatMoney(item.unit_cost)"></td>
                                 <td class="py-2.5 px-3 text-right font-mono font-semibold text-ink"
                                     x-text="'Rs ' + formatMoney(item.qty * item.unit_cost)"></td>
                             </tr>
                         </template>
                     </tbody>
                 </table>
                 ...
                 <div class="w-1/2 p-3 bg-paper border border-line rounded-[var(--radius-s)] space-y-1.5 text-xs">
                     ...
                     <div class="flex justify-between text-muted">
                         ...
                         <span class="font-mono font-semibold text-ink"
                             x-text="'Rs ' + formatMoney(activePurchase?.total_amount)"></span>
                     </div>
                     <div class="flex justify-between text-muted">
                         <span>Amount Paid:</span>
                         <span class="font-mono font-semibold text-ink"
                             x-text="'Rs ' + formatMoney(activePurchase?.supplier_payment.amount)"></span>
                     </div>
                     <div class="flex justify-between text-sm font-bold border-t border-line pt-2 text-ink">
                         <span>Total Payable:</span>
                         <span class="font-mono text-forest"
                             x-text="'Rs ' + formatMoney(activePurchase?.remaining_amount)"></span>
                     </div>
                 </div>
             </div>
         </div>
