...
<head>
    ...
    <title>Purchase Bill - {{ $purchase->invoice_number }}</title>
    ...
    <div x-data="billSwitcher()" class="min-h-screen p-5 md:p-8">
        <div class="max-w-[900px] mx-auto space-y-6">
            ...
            <div class="print:hidden flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <span class="text-xs font-semibold uppercase tracking-wider text-muted">Procurement &
                        Inventory</span>
                    <h1 class="font-display text-2xl font-bold text-forest">
                        Purchase Order — <span class="font-mono">{{ $purchase->invoice_number }}</span>
                    </h1>
                </div>
            ...
            <div x-show="format === 'thermal'" id="bill-thermal-wrap">
                @include('purchases.partials.bill-thermal', ['purchase' => $purchase])
            </div>

            <!-- A4 Invoice (Optional placeholder) -->
            {{-- <div x-show="format === 'a4'" id="bill-a4-wrap">
                @include('purchases.partials.bill-a4', ['purchase' => $purchase])
            </div> --}}
