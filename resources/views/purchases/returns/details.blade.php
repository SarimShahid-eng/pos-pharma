    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <h1 class="font-display text-2xl font-bold text-forest">
                    Return History — <span class="font-mono">{{ $purchase->invoice_number }}</span>
                </h1>
            </div>
        ...
        <div class="panel bg-white">
            <div class="grid grid-cols-2 md:grid-cols-6 gap-4">
                <div>
                    ...
                    <p class="text-sm font-mono font-medium">{{ $purchase->date->format('d M, Y') }}</p>
                </div>
                <div>
                    <p class="text-xs font-semibold uppercase tracking-wider text-muted mb-1">Supplier</p>
                    <p class="text-sm font-mono font-medium">{{ $purchase->supplier->name ?? 'N/A' }}</p>
                </div>
                <div>
                    <p class="text-xs font-semibold uppercase tracking-wider text-muted mb-1">Reference #</p>
                    <p class="text-sm font-mono font-medium">{{ $purchase->reference_number ?? '—' }}</p>
                </div>
                <div>
                    <p class="text-xs font-semibold uppercase tracking-wider text-muted mb-1">Original Net Total</p>
                    <p class="text-sm font-mono font-bold text-ink">Rs {{ number_format($purchase->total_amount, 2) }}</p>
                </div>
                <div>
                    ...
                    <p class="text-sm font-mono font-bold text-tag-red">
                        Rs {{ number_format($purchase->purchaseReturns->sum('received_amount'), 2) }}
                    </p>
                </div>
                <div>
                    <p class="text-xs font-semibold uppercase tracking-wider text-muted mb-1">Return Events</p>
                    <p class="text-sm font-mono font-bold text-forest">{{ $purchase->purchaseReturns->count() }}</p>
                </div>
            </div>
        </div>
                ...
                <table class="w-full text-left text-sm text-ink">
                    <thead
                        ...
                        <tr>
                            ...
                            <th class="py-3 px-4 w-28 text-center">Originally Purchased</th>
                    ...
                    <tbody class="divide-y divide-line">
                        @foreach ($purchase->purchaseItems as $item)
                            <tr class="{{ $item->max_returnable_qty <= 0 ? 'opacity-50' : '' }}">
                                ...
                                <td class="py-2.5 px-4 text-right font-mono">Rs {{ number_format($item->unit_cost, 2) }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
        ...
        <div class="space-y-4">
            <h2 class="text-xs font-bold uppercase tracking-wider text-forest px-1">
                Return Events ({{ $purchase->purchaseReturns->count() }})
            </h2>

            @forelse ($purchase->purchaseReturns->sortByDesc('date') as $return)
                <div
                    class="panel bg-white p-0 overflow-hidden {{ $return->id === $purchaseReturn->id ? 'ring-2 ring-forest' : '' }}">
                    <div
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            @if ($return->id === $purchaseReturn->id)
                                <span
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <a href="{{ route('purchases.bill.purchaseReturn.receipt', $return->id) }}"
                                ...
                                @foreach ($return->purchaseReturnItems as $returnItem)
                                    <tr>
                                        ...
                                        <td class="py-2 px-4 text-right font-mono">Rs
                                            {{ number_format($returnItem->unit_cost, 2) }}</td>
