...
@section('content')
    <div x-data="purchaseIndex()" class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <h1 class="font-display text-2xl font-bold text-forest">Purchase Returns</h1>
            </div>
        ...
        <x-search-bar-filter :route="route('purchases.returns.index')" placeholder="Search by invoice number...">
                ...
                <table class="w-full text-left text-sm text-ink">
                    <thead class="bg-paper border-b border-line text-xs font-semibold uppercase tracking-wider text-muted">
                        <tr>
                            ...
                            <th class="py-3 px-4 text-right">Total Purchased Amount</th>
                        ...
                        @forelse($purchaseReturns as $purchaseReturn)
                            <tr class="hover:bg-paper/50 transition-colors">
                                ...
                                <td class="py-3 px-4 font-mono text-xs text-muted">
                                    {{ $purchaseReturn->formatted_date }}
                                </td>

                                <!-- Total purchase Items -->
                                <td class="py-3 px-4 text-center font-mono text-xs text-ink font-semibold">
                                    {{ $purchaseReturn->purchase->purchaseItems->sum('qty') }}
                                </td>
                                <!-- Total Return Items -->
                                <td class="py-3 px-4 text-center font-mono text-xs text-ink font-semibold">
                                    {{ $purchaseReturn->purchaseReturnItems->sum('qty') }}
                                </td>
                                <!-- Total Return Items -->
                                <td class="py-3 px-4 text-center font-mono text-xs text-ink font-semibold">
                                    {{ $purchaseReturn->purchase->purchaseItems->sum('qty') - $purchaseReturn->purchaseReturnItems->sum('qty') }}
                                </td>

                                <!-- Total Amount -->
                                <td class="py-3 px-4 text-right font-mono text-xs font-bold text-ink">
                                    Rs {{ number_format($purchaseReturn->purchase->total_amount, 2) }}
                                </td>
                                <!-- Total Amount -->
                                <td class="py-3 px-4 text-right font-mono text-xs font-bold text-ink">
                                ...
                                <td class="py-3 px-4 text-right whitespace-nowrap">
                                    <div class="flex items-center justify-end gap-2">
                                        {{-- <button type="button"
                                            @click="openModal({{ json_encode($purchaseReturn->load(['supplier', 'purchaseReturnItems.product'])) }})"
                                        ...
                                        <!-- Return Purchase -->
                                        {{-- <button type="button"
                                            @click="openReturnModal({{ json_encode($purchaseReturn->load(['supplier', 'purchaseReturnItems.product'])) }})"
                                        ...
                                        <a href="{{ route('purchases.returns.details', $purchaseReturn) }}"
                                            class="rounded-[var(--radius-s)] border border-line bg-white p-1.5 text-muted hover:text-forest hover:border-forest transition-colors"
                                            title="View Details">
                                            <svg class="w-4 h-4 stroke-current fill-none" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <a href="{{ route('purchases.bill.purchaseReturn.receipt', $purchaseReturn) }}" target="_blank"
                                            class="rounded-[var(--radius-s)] border border-line bg-white p-1.5 text-muted hover:text-tag-red hover:border-tag-red transition-colors"
                                            title="Bill">
        ...
        @include('purchases.return_modal') --}}
    ...
    <script>
        ...
        //             this.returnPurchase = purchase;
        //             this.returnError = '';
        //             this.returnSuccess = false;
        //             this.returnSubmitting = false;
        //             this.returnReason = purchase.purchase_return ? purchase.purchase_return.notes : '';
        //             this.returnDate = purchase?.purchase_return ? purchase.purchase_return.return_date :
        //                 new Date()
        //                 .toISOString().split('T')[0];;
        //             this.returnItems = (purchase.purchase_items || []).map(pi => ({
        ...
        //             this.returnFormAction = "{{ route('purchases.returns.store') }}";
        ...
        //             this.returnPurchase = null;
        //             this.returnItems = [];
        //             this.returnError = '';
        //             this.returnSuccess = false;
        //         },

        //         // Keeps each return qty within [0, purchased_qty] as the user types
        //         clampReturnQty(index) {
        //             let item = this.returnItems[index];
        //             let max = item.purchased_qty;
        ...
        //             let purchased = parseInt(item.purchased_qty) || 0;
        //             let returning = parseInt(item.return_qty) || 0;

        //             return Math.max(0, purchased - returning);
        //         },
        ...
        //                 if (qty <= 0 || qty > item.purchased_qty) {
        //                     this.returnError =
        //                         `Return quantity for "${item.product_name}" must be between 1 and ${item.purchased_qty}.`;
        //                     return;
        //                 }
        //             }
        ...
        //                         })),
        //                         purchase_id: this.returnPurchase.id,
